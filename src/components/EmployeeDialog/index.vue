<template>
	<yx-dialog title="选择员工" :bottomBtn="bottomBtn" :visible.sync="visible">
		<div>
			<yx-form
				:inline="true"
				:data="dialogData"
				:items.sync="dialogCfg"
				:formAction="dialogAction"
			>
			</yx-form>
			<el-row>
				<yx-table
					v-loading="showLoading"
					:columns="tialogColumns"
					:pagination="tialogPage"
					:data="tialogData"
				>
				</yx-table>
			</el-row>
		</div>
	</yx-dialog>
</template>

<script>
import YxDialog from '@/components/Dialog/index.vue'
import YxForm from '@wg-vue-materials/yx-form'
import YxTable from '@wg-vue-materials/yx-table'
import { getClerkListByCondition } from '@/api/clerk'
export default {
	components: {
		YxDialog,
		YxForm,
		YxTable,
	},
	props: {
		// 弹框开关
		dialogVisible: {
			type: Boolean,
			default: false,
		},
		// 机构
		selectedList: {
			type: Array,
			default: () => [],
		},
	},
	data() {
		return {
			showLoading: false,
			visible: false,
			bottomBtn: [
				{
					title: '确定',
					click: () => {
						this.saveCoupon()
					},
				},
			],
			dialogData: {
				// 选择员工搜索条件
				keyword: '', // 员工名称
			},
			dialogCfg: [
				// 员工配置项
				{
					type: 'input',
					dataIndex: 'keyword',
					title: '筛选',
				},
			],
			dialogAction: [
				// 员工按钮
				{
					title: '查询',
					type: 'primary',
					click: () => {
						this._getValidCouponPage(true)
					},
				},
			],
			tialogColumns: [
				// 员工配置项
				{
					type: 'string',
					dataIndex: 'userName',
					title: '用户名',
				},
				{
					type: 'string',
					dataIndex: 'clerkName',
					title: '员工名称',
				},
				{
					type: 'string',
					dataIndex: 'departmentName',
					title: '部门',
				},
				{
					type: 'action',
					title: '操作',
					width: '150',
					actions: [
						{
							title: '选择',
							click: (row) => {
								this.saveCoupon(row)
							},
						},
					],
				},
			],
			tialogPage: {
				count: 0,
				length: 10,
				currentPage: 1,
				tableChange: () => {
					this._getValidCouponPage()
				},
			},
			tialogData: [],
		}
	},
	methods: {
		saveCoupon(row) {
			this.selectedList.unshift(row)
			this.visible = false
		},
		findCouponIndex(row) {
			// 判断是否已经添加员工
			let selectCouponList = this.selectedList.map((item) => {
				return item.yxCouponId
			})
			return selectCouponList.indexOf(row.yxCouponId) == -1
		},
		findCounponParams(flag) {
			// 员工搜索条件
			let paramJson = Object.assign(this.dialogData, {
				startIndex: 0,
				length: 1,
				userState: 1,
			})
			// 每页的加载数据长度
			paramJson.length = this.tialogPage.length
			// 当前页的条数开始
			if (flag) {
				this.tialogPage.currentPage = 1
			}
			paramJson.startIndex =
				(this.tialogPage.currentPage - 1) * this.tialogPage.length
			return paramJson
		},
		async _getValidCouponPage(flag) {
			// 员工列表
			this.showLoading = true
			let paramJson = this.findCounponParams(flag)
			let {
				data: { resultCode, resultData },
			} = await getClerkListByCondition(paramJson)
			this.showLoading = false
			if (resultCode == 0) {
				this.tialogData = resultData.clerkList
				this.tialogPage.count = resultData.clerkCount
			} else {
				this.tialogPage.count = 0
				this.tialogData = []
			}
		},
	},
	watch: {
		visible(newVlaue) {
			this.$emit('update:dialogVisible', newVlaue)
			if (this.visible) {
				this._getValidCouponPage(true)
			}
		},
		dialogVisible(newVlaue) {
			if (this.isShow !== newVlaue) {
				this.visible = newVlaue
			}
		},
	},
}
</script>
