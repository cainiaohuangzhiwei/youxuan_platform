...
export default {
	...
	methods: {
		...
		async init(flag) {
			...
			this.loading = true
			const res = await findSupplierAccountNumberForList(paramJson)
		...
		submitForm() {
			...
			if (this.chooseType !== 'radio') {
				const list = this.supplierAccountNumberList.map((item) => {
				...
				this.rowData.supplierName = list
					.map((item) => {
						return item.supplierName
					})
					.join('，')
				this.rowData.supplierAccountNumberIds = list.map((item) => {
