...
import {
	getCouponAmountListByCondition,
	exportCouponAmountListByCondition,
...
export default {
		...
		return {
			...
			formAction107: [
				...
				},
				{
					title: '导出Excel',
					type: 'warning',
					auth: 'exportCouponAmountListByCondition',
					click: () => {
						this.exportExcel()
					},
				},
	...
	methods: {
		...
		exportExcel: async function () {
			...
			let { data } = await exportCouponAmountListByCondition(paramJson)
			loading.close()
		...
		async getData108(flag) {
			...
			} = await getCouponAmountListByCondition(paramJson)
			if (resultCode == 0) {
				this.accountItems109[0].value =
					resultData.flowStatistics.orderAmountCount || 0
				this.accountItems109[1].value =
					resultData.flowStatistics.realAmountCount || 0
				this.accountItems109[2].value =
					resultData.flowStatistics.couponAmountCount || 0
				this.tableData108 = resultData.couponAmountList
