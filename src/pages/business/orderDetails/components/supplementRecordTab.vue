<template>
	<div>
				...
				<yx-info :column="3" :data="item" :items="refundItems1">
					...
					<template slot="refundTime">
						<div class="info-item">
							...
							<span>
								{{
									item.compensationPlatformType == 4
										? '平台发起'
										: item.compensationPlatformType == 8
										? '商家发起'
										: '-'
								}}
							</span>
		...
		<!-- 赔付信息 -->
		<template v-for="(item, index) in compensationList">
			<div :key="index" class="border-box mt20">
				<yx-info :column="2" :data="item" :items="compensationItems">
					<template slot="remarkType">
						<div class="info-item">
							<span>类型：</span>
							<span>
								{{
									item.remarkType == 21 ? '运费保障赔付-补偿到微信零钱' : '-'
								}}
							</span>
						</div>
					</template>
...
import { queryJoinpayRefund } from '@/api/yuntongPay'
...
export default {
	...
	methods: {
				...
				this.$confirm('查询退款是否成功', '退款查询', {
					...
					callback: async (action) => {
						if (action === 'confirm') {
							...
							} = await queryJoinpayRefund({
								refundNo: item.refundSn,
								orderId: item.orderId,
							})
							if (resultCode == 0) {
								let ra_Status = resultData.refundResult.ra_Status
								let rc_CodeMsg = resultData.refundResult.rc_CodeMsg
		...
		//退货退款原因：1. 质量问题;2. 缺货退款;3. 特殊处理;4. 拒收退款;5. 取消订单；补款原因：6. 缺货退款+补偿
		// 7. 漏发少件补偿+退货/退款8. 特殊处理9. 退货运费10. 质量问题'';// 11.七天无理由;12.漏发/少发/未收到货;13.错发退款;14.物流停滞/异常/丢件;
		// 15.大小/尺码与描述不符 16.供应商同意退款 17.超售后期退款// 18.超时支付 19.破损补款 20.未按约定时间发货
		// 获取退款类型
		...
		refundPlatform(row) {
			if (row.refundMode == 5) {
				let supplementTypeDesc =
					row.supplementType == 2
						? '奖励到店主余额'
						: row.supplementType == 3
						? '奖励到银行卡'
						: '奖励到零钱'
				let refundType =
					row.refundPlatform == 1
						? '用户发起'
						: row.refundPlatform == 2
						? '后台发起'
						: '其他'
				return `${refundType}-订单补款-${supplementTypeDesc}`
			} else {
