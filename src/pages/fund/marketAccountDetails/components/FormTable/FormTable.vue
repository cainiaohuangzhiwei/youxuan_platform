<template>
	<div>
		<Info v-auth="'platformMarketingAccountDetail'" />
		<div v-auth="'platformMarketingBalanceChangeList'">
			<yx-form
				:inline="true"
				:data="searchData"
				:items="searchCfg"
...
import { platformMarketingBalanceChangeList } from '@/api/platformAccount'
...
export default {
	...
	data() {
		return {
			platformMarketingBalanceChangeList: checkFunctionCode(
				'platformMarketingBalanceChangeList'
			),
			incomeSum: 0,
			expensesSum: 0,
			showLoading: false,
			...
			searchAction: [
				{
					...
					type: 'primary',
					click: (e) => {
						this.getPlatformMarketingBalanceChangeList(true, e)
					},
				},
				{
					title: '重置',
			...
			page: {
				...
				currentPage: 1,
				tableChange: () => {
					this.getPlatformMarketingBalanceChangeList()
				},
			},
		}
	},
	...
	methods: {
		...
		getPlatformMarketingBalanceChangeList: async function (flag, e) {
			if (!this.platformMarketingBalanceChangeList) return
			...
			} = await platformMarketingBalanceChangeList(paramJson, custom)
			this.showLoading = false
			if (resultCode == 0) {
				this.tableData = accountList || []
				this.incomeSum = numberFormat(incomeSum, 2)
				this.expensesSum = numberFormat(expensesSum, 2)
