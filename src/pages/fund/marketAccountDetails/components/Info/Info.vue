...
import { platformMarketingAccountDetail } from '@/api/platformAccount'
export default {
	components: {
		YxInfo,
	},
	...
	methods: {
		platformMarketingAccountDetail: async function () {
			let paramJson = {
				accountId: this.$route.params.accountId,
			}
			let {
				data: { resultCode, resultData },
			} = await platformMarketingAccountDetail(paramJson)
			if (resultCode == 0) {
				this.infoData = {
					accountId: this.$route.params.accountId,
					...resultData,
