...
import { getMerchantBankAccountByFlowWithdrawId } from '@/api/yhtPlusCmsBankAccount'
export default {
	components: {
		YxDialog,
		YxForm,
	...
	methods: {
		_getMerchantBankAccountByFlowWithdrawId: async function () {
			// 获取详细信息
			this.showLoading = true
			let {
				data: { resultCode, resultData = {} },
			} = await getMerchantBankAccountByFlowWithdrawId({
	...
	watch: {
		...
		dialogVisible(newVlaue) {
			...
			if (this.visible) {
				this._getMerchantBankAccountByFlowWithdrawId()
			}
		},
	},
}
