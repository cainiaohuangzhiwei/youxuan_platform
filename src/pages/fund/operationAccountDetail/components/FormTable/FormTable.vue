...
import {
	accOperationsAccountList,
	accOperationsAccountAmountSum,
...
export default {
	...
	data() {
		return {
			...
			showLoading: false,
			searchData: {
				beginAddTime: moment().startOf('day').valueOf(), // 开始时间
			...
			searchCfg: [
				{
					title: '变动时间',
					type: 'datetimerange',
					dataIndex: ['beginAddTime', 'endAddTime'],
				},
				{
					type: 'input',
					title: '业务单号',
				...
				},
				{
					...
					label: 'value',
					options:
						this.$store.state.app.constant.WATER_TYPE_OPERATION_ACCOUNT_DETAIL,
				},
			],
			searchAction: [
				{
					...
					type: 'primary',
					click: (e) => {
						this.getAccOperationsAccountList(true, e)
					},
				},
				{
					title: '重置',
			...
			page: {
				...
				currentPage: 1,
				tableChange: () => {
					this.getAccOperationsAccountList()
				},
			},
		}
	},
	...
	methods: {
		...
		async _accOperationsAccountAmountSum(paramJson) {
			let {
				data: { resultCode, resultData },
			} = await accOperationsAccountAmountSum(paramJson)
			if (resultCode == 0) {
				this.incomeSum = numberFormat(resultData.income || 0, 2)
				this.expensesSum = numberFormat(resultData.expense || 0, 2)
			}
		},
		getAccOperationsAccountList: async function (flag, e) {
			// if (!this.platformWeegetBalanceChangeList) return
			this.showLoading = true
			let paramJson = this.findSearchParams(flag)
			this._accOperationsAccountAmountSum(paramJson)
			let custom = e
				? {
			...
			let {
				data: { resultCode, resultData },
			} = await accOperationsAccountList(paramJson, custom)
