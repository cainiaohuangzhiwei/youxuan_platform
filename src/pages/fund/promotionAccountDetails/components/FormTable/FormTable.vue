<template>
	<div>
		<!-- v-auth="'platformWeegetAccountDetail'" -->
		<Info />
		<!-- v-auth="'platformWeegetBalanceChangeList'" -->
		<div>
			<yx-form
				:inline="true"
				:data="searchData"
...
import {
	accPromotionAccountList,
	accPromotionAccountAmountSum,
...
export default {
	...
	data() {
		return {
			...
			searchAction: [
				{
					...
					type: 'primary',
					click: (e) => {
						this.getAccPromotionAccountList(true, e)
					},
				},
				{
					title: '重置',
			...
			page: {
				...
				currentPage: 1,
				tableChange: () => {
					this.getAccPromotionAccountList()
				},
			},
		}
	},
	...
	methods: {
		...
		async _accPromotionAccountAmountSum(paramJson) {
			let {
				data: { resultCode, resultData },
			} = await accPromotionAccountAmountSum(paramJson)
			if (resultCode == 0) {
				this.incomeSum = numberFormat(resultData.income || 0, 2)
				this.expensesSum = numberFormat(resultData.expense || 0, 2)
			}
		},
		getAccPromotionAccountList: async function (flag, e) {
			// if (!this.platformWeegetBalanceChangeList) return
			this.showLoading = true
			let paramJson = this.findSearchParams(flag)
			this._accPromotionAccountAmountSum(paramJson)
			let custom = e
				? {
			...
			let {
				data: { resultCode, resultData },
			} = await accPromotionAccountList(paramJson, custom)
