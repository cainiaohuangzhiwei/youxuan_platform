<template>
	<div>
		...
		>
			...
			<span> 说明：请下载属性导入模板，按照属性导入模板说明进行导入 </span>
		</el-dialog>
	</div>
</template>

<script>
import { importBasicsAttribute } from '@/api/yhtPlusCmsGoodsCategoryAttribute'
...
export default {
	...
	data() {
		return {
			...
			uploadImageList: '/yhtplus/webImport/importBasicsAttribute.action',
		}
	},
	...
	methods: {
		...
		async submitForm() {
			...
			const res = await importBasicsAttribute(formData)
		...
		// 下载Excel模板
		handleDownExcel() {
			downloadFile(`/excel/goods/属性库导入模板.xlsx`)
		},
		...
		async uploadImageHttpRequest(params) {
			...
			// const res = await importAuditedSupplierGoods(formData)
			// console.log(res, 'ssfsdfsdfsdfsd')
			// this.batchData.cusServiceTrusteeshipList = []
			// if (
