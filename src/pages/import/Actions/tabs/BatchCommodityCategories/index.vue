...
import {
	exportBatchGoodsCategoryDetails,
	batchEditGoodsCategory,
...
export default {
			...
			storeActions: [
				{
					title: '下载商品信息',
					type: 'primary',
					// auth: 'exportBatchGoodsCategoryDetails',
					click: (e) => {
						this._exportBatchGoodsCategoryDetails(e)
					},
				},
				{
					title: '查看导入记录',
			...
			storeDownloadOptions: [
				{
					...
					uploadAttributes: {
						...
						httpRequest: (file) => this._batchEditGoodsCategory(file),
					},
					class: 'excel-upload',
				},
			],
	...
	methods: {
		importHistory() {
			this.$router.push({ path: '/import/categoryImportRecord' })
		},
		async _exportBatchGoodsCategoryDetails(e) {
			...
			let { data } = await exportBatchGoodsCategoryDetails(paramJson, custom)
			downloadFile(data, '商品信息.xlsx')
			this.brandData = ''
		},
		openBrandDialog() {
			this.$emit('openBrandDialog', 'BatchCommodityCategories')
		},
		...
		async _batchEditGoodsCategory(file) {
			...
			} = await batchEditGoodsCategory(formData)
			if (resultCode === 0) {
