...
import { goodsMaterialUpdate } from '@/api/newMaterial'
...
export default {
	...
	watch: {
		localVisible(val) {
			...
			if (val) {
				this.form.materialSource = this.singeMaterialRow.materialSource
				this.form.effectivityType = this.singeMaterialRow.effectivityType
				if (
					this.singeMaterialRow.picMaterialContents &&
					this.singeMaterialRow.picMaterialContents.length
				) {
					this.form.imgList = JSON.parse(
						JSON.stringify(this.singeMaterialRow.picMaterialContents)
					)
					this.form.fileList = JSON.parse(
						JSON.stringify(this.singeMaterialRow.picMaterialContents)
					)
				}
				this.form.shopId = this.singeMaterialRow.shopId || ''
				this.form.virtualShopId = this.singeMaterialRow.virtualShopId || ''
				this.form.virtualShopkeeperName =
					this.singeMaterialRow.virtualShopkeeperName || ''
				this.form.virtualShopkeeperPortraitUrl =
					this.singeMaterialRow.virtualShopkeeperPortraitUrl || ''
				this.form.sort = this.singeMaterialRow.sort
				this.form.status = this.singeMaterialRow.status
				this.form.wordMaterialContent =
					this.singeMaterialRow.wordMaterialContent
				if (this.form.effectivityType == 1) {
					this.form.time = [
						new Date(this.singeMaterialRow.effectivityStartTime),
						new Date(this.singeMaterialRow.effectivityEndTime),
					]
				}
			}
		},
	...
	methods: {
		...
		async handleFix() {
			...
			let params = {
				activityGoodsMaterialRefId:
					this.singeMaterialRow.activityGoodsMaterialRefId,
			...
			if (this.form.materialSource == 1) {
				...
				params.virtualShopId = this.form.virtualShopId
				params.virtualShopkeeperName = this.form.virtualShopkeeperName
				params.virtualShopkeeperPortraitUrl =
					this.form.virtualShopkeeperPortraitUrl
			}
			...
			if (this.form.imgList && this.form.imgList.length) {
				params.picMaterialContents = this.form.imgList
			}
			let {
				data: { resultCode },
			} = await goodsMaterialUpdate(params)
			this.fixLoading = false
			if (resultCode == 0) {
				this.$message.success('编辑成功')
				this.localVisible = false
				this.$emit('singeRefresh')
			}
		},
		datadragEnd(evt) {
			console.log(evt, 'evt')
