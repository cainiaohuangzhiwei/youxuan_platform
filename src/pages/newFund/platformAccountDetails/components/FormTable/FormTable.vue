<template>
	<div>
		<el-card class="box-card">
			...
			<div class="c-money">
				<div v-if="$route.params.type == 1 || $route.params.type == 3">
					<el-tooltip
						class="item"
						effect="dark"
		...
		<el-card>
			...
			<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
				...
				<el-tab-pane v-if="$route.params.type == 2" label="垫资记录" name="two">
					<pad-support
						v-if="activeName == 'two'"
						ref="padSupport_ref"
...
import {
	baseAccountBalancePrice,
	marketingAccountBalancePrice,
	allianceAccountBalancePrice,
	joinpayAllianceAccountBalancePrice,
} from '@/api/newFund'
import { wgGetDate, numberFormat, setViewTitle } from '@/utils/helper/common'
import pDetails from '../details.vue'
import padSupport from '../padSupport.vue'
			...
			searchAction: [
				{
					...
					click: () => {
						if (this.activeName != 'two') {
							this.$refs.pDetails_ref.init(true)
						} else {
							this.$refs.padSupport_ref.init(true)
						}
					},
				},
				{
	...
	methods: {
		...
		async balancePrice() {
			...
			if (this.$route.params.type == 1) {
				let { data } = await baseAccountBalancePrice({
				...
				let { data } = await marketingAccountBalancePrice({
					accountSn: this.$route.params.id,
					accountType: this.$route.params.type,
				})
				resultData = data.resultData || {}
				...
				let { data } = await allianceAccountBalancePrice({
					accountSn: this.$route.params.id,
					accountType: this.$route.params.type,
				})
				resultData = data.resultData || {}
				...
				let { data } = await joinpayAllianceAccountBalancePrice({
					accountSn: this.$route.params.id,
					accountType: this.$route.params.type,
				})
