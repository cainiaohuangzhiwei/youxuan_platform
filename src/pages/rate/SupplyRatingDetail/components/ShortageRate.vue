...
import {
	getSupplierNoGoodRateGradeList,
	getSupplierNoGoodRateGradeListExport,
...
export default {
	...
	data() {
		return {
			...
			tableColumns: [
				...
				{
					type: 'string',
					dataIndex: 'noGoodFlag',
					title: '是否缺货',
					customRender: (row) => {
						return row.noGoodFlag == 1 ? '是' : row.noGoodFlag == 2 ? '否' : '-'
					},
				},
				{
					type: 'string',
			...
			formCfg: [
...

				{
					...
					options: [
						{ noGoodFlag: 0, label: '全部' },
						{ noGoodFlag: 1, label: '是' },
						{ noGoodFlag: 2, label: '否' },
					],
				},
				{
					title: '订单时间',
	...
	methods: {
					...
					formatter: function (params) {
						...
						for (let i = 0; i < params.length; i++) {
							res +=
								...
								params[i].seriesName +
								'：' +
								(params[i].seriesName.indexOf('缺货率') != -1
									? params[i].data * 100
									: params[i].data) +
								(params[i].seriesName.indexOf('缺货率') != -1 ? '%' : '')
						}
		...
		exportExcel: async function () {
			...
			this.eLoading = true
			let { data } = await getSupplierNoGoodRateGradeListExport(paramJson)
			this.eLoading = false
		...
		async _init(flag) {
			...
			} = await getSupplierNoGoodRateGradeList(paramJson)
