<template>
	<div class="page-wrapper">
		...
		<AddResourceDialog
			...
			:formDisabled="addDialogTitle === '查看浮窗广告'"
...
import { getFloatWindowPage, getStartPageActivityDate } from '@/api/marketing'
...
export default {
		...
		return {
			...
			formAction: [
				...
				},
				{
					title: '新增',
					type: 'success',
					auth: 'advertisementFloatWindowPageAdd',
			...
			tableColumns: [
				...
				},
				{
					...
					actions: [
						{
							title: '编辑',
							auth: 'advertisementFloatWindowPageUpdate',
						...
						{
							title: '数据',
							auth: 'advertisementFloatWindowPageGetActivityDate',
							customRender: (action, row) => {
								// 未开始隐藏
	...
	methods: {
		...
		async handleSearch(flag) {
			...
			} = await getFloatWindowPage(paramJson).catch(() => {
				this.loading = false
			})
		...
		handleAdd() {
			this.addDialogTitle = '新建浮窗广告'
			this.addDialogCurrentRowData = {}
			this.addDialogVisible = true
		},

		handleEditItem(row) {
			this.addDialogTitle = '编辑浮窗广告'
			this.addDialogCurrentRowData = row
			this.addDialogVisible = true
		},

		handleViewItem(row) {
			this.addDialogTitle = '查看浮窗广告'
			this.addDialogCurrentRowData = row
			this.addDialogVisible = true
		},
