<template>
	<div class="page-wrapper">
		...
		<AddPopupDialog
			...
			:formDisabled="addDialogTitle === '查看弹窗广告'"
...
import { getPopUpsWindowPage, getStartPageActivityDate } from '@/api/marketing'
...
export default {
		...
		return {
			...
			formAction: [
				...
				},
				{
					title: '新增',
					type: 'success',
					auth: 'advertisementPopUpsWindowPageAdd',
			...
			tableColumns: [
				...
				},
				{
					...
					title: '应用',
					customRender: ({ applicationModuleList }) => {
						return applicationModuleList.join('、') || '-'
					},
				},
				{
					type: 'string',
				...
				},
				{
					...
					actions: [
						{
							title: '编辑',
							auth: 'advertisementPopUpsWindowPageUpdate',
						...
						{
							title: '数据',
							auth: 'advertisementPopUpsWindowPageGetActivityDate',
							customRender: (action, row) => {
								// 未开始隐藏
	...
	methods: {
		...
		async handleSearch(flag) {
			...
			} = await getPopUpsWindowPage(paramJson).catch(() => {
				this.loading = false
			})
		...
		handleAdd() {
			this.addDialogTitle = '新建弹窗广告'
			this.addDialogCurrentRowData = {}
			this.addDialogVisible = true
		},

		handleEditItem(row) {
			this.addDialogTitle = '编辑弹窗广告'
			this.addDialogCurrentRowData = row
			this.addDialogVisible = true
		},

		handleViewItem(row) {
			this.addDialogTitle = '查看弹窗广告'
			this.addDialogCurrentRowData = row
			this.addDialogVisible = true
		},
