...
import {
	addPopUpsWindowPage,
	updatePopUpsWindowPage,
	getPopUpsWindowDetail,
} from '@/api/marketing'
...
export default {
	...
	watch: {
		async visible(val) {
			...
			// 应用场景回显
			if (defaultValue.advApplicationSceneList) {
				defaultValue.applicationModuleList =
					defaultValue.advApplicationSceneList?.map((i) => i.applicationModule)
			}
	...
	methods: {
		...
		async handleConfirm() {
			...
			const requestParams = {
				...
				applicationModuleList: params.applicationModuleList || [],
			...
			// 新增/编辑
			// 启动广告新增和更新
			let fetchMethods = params.advActivityId
				? updatePopUpsWindowPage
				: addPopUpsWindowPage
			...
			if (params.advActivityId) {
				...
				requestParams.advApplicationSceneList =
					params.applicationModuleList.map((i) => {
						...
						return {
							advApplicationSceneId: target ? target.advApplicationSceneId : 0,
							applicationModule: i,
						}
					})
