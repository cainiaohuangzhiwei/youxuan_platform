...
export default {
			...
			searchCfg: [
				{
					...
					dataIndex: 'payType',
					label: 'value',
					options: this.$store.state.app.constant.DIRECT_PAY_STATE,
				},
				{
					type: 'select',
					title: '状态',
	...
	methods: {
		...
		handleExpiryTime(row) {
			if (row.payType == 4 || row.payType == 7 || row.payType == 8) {
		...
		handlePayType(row) {
			...
			if (row.payType == 7 || row.payType == 8) {
