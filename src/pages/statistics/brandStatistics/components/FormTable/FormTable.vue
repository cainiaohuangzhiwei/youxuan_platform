...
import {
	getCMSGoodsGroupOrderStatistics,
	exportCMSGoodsGroupOrderStatistics,
...
export default {
	...
	methods: {
		...
		async exportIt() {
			...
			let { data } = await exportCMSGoodsGroupOrderStatistics(paramJson)
		...
		async getData102(flag) {
			...
			} = await getCMSGoodsGroupOrderStatistics(paramJson)
			if (resultCode != 0) {
				this.$message({
					type: 'error',
					message: '查询超时',
