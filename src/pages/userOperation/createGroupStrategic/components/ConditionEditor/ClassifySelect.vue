...
import { queryVisibleUoAttributeClassifyBatch } from '@/api/uoOptionKey'
...
export default {
	...
	watch: {
		localVisible(val) {
			if (val) {
				this.selector = {
					...this.classifyItem,
					name: this.classifyItem.classifyName,
				}
				this.getData()
			}
		},
	...
	methods: {
		...
		findIndex(row) {
			// 判断是否已经添加用户
			return this.selector.uoAttributeClassifyId === row.uoAttributeClassifyId
		},
		updatedSelector(row) {
			// 添加/移除用户
			if (this.selector.uoAttributeClassifyId !== row.uoAttributeClassifyId) {
				this.selector = row
			} else {
				this.selector = {}
			}
		...
		getData: async function (flag) {
			...
			} = await queryVisibleUoAttributeClassifyBatch(paramJson)
			if (resultCode == 0) {
