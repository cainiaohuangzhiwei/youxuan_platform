<template>
	...
	>
		...
		<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
			<el-tab-pane :disabled="this.isCheckView" label="选择商品" :name="1"
				><el-row>
			...
			<el-tab-pane
				...
				:label="'已选商品' + selectGroupList.length"
				:name="2"
				><el-row>
					<el-button
...
import {
	queryGoodsGroupInfoFilterAKCBatch,
...
export default {
	...
	watch: {
		localVisible(val) {
			this.groupList = {}
			this.activeName = this.activeName = this.isCheckView ? 2 : 1
			...
			if (this.inquireProductRow.length > 0 && this.BooleS) {
				...
				this.groupList.brandId = brandIdS
	...
	methods: {
		...
		handleRemovalSd() {
			...
			for (let i = 0; i < this.selectionDataSd.length; i++) {
				for (let j = 0; j < this.selectGroupList.length; j++) {
					if (
						String(this.selectionDataSd[i].goodsId) ===
						String(this.selectGroupList[j].goodsId)
					) {
						const indexs = this.selectGroupList.findIndex(
							(item) => item.goodsId === this.selectGroupList[j].goodsId
						)
						this.selectGroupList.splice(indexs, 1)
					}
				}
			}
			console.log('商品范围去移除', this.selectGroupList)
		...
		findSearchParams188(value) {
			...
			paramJson.pageSize = paramJson.length
			paramJson.pageNum = paramJson.startIndex / paramJson.length + 1
			paramJson.groupName = value
			return paramJson
		},
		//远程搜索
		...
		findUserParams(flag) {
			...
			const paramJson = Object.assign(
				{
					goodsGroupIdList:
						...
						!this.BooleS
							...
							: [''],
					brandIdList:
						(this.groupList.brandId && this.inquireProductRow.length === 0) ||
						!this.BooleS
							? [this.groupList.brandId]
							: this.inquireProductRow.length > 0
							? this.groupList.brandId
							: [],
					viewType: 3,
			...
			paramJson.brandIdList = paramJson.brandIdList.filter((item) => item)
			return paramJson
		},
