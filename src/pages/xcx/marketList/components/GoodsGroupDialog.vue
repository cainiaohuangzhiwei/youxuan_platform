...
export default {
	...
	watch: {
		localVisible(val) {
			...
			if (val) {
				...
				if (this.formData.actDiscountId) {
					this.actDiscountId = this.formData.actDiscountId
					if (this.formData.supplierId) {
						this.searchData.supplierId = this.formData.supplierId
						this.selectedSupplier.supplierId = this.formData.supplierId
	...
	methods: {
		...
		async _getGoodsGroupListByConditionForCMS(flag) {
			...
			this.loading = true
			const res = await getGoodsGroupListByConditionForCMS(paramJson)
		...
		findIndex(row) {
			const { actDiscountId } = this.formData || {}
			// 判断是否已经添加
			let remove = this.selectGroupList.some(
				(e) => Number(e.goodsGroupId) === Number(row.goodsGroupId)
			)
			// groupState 0：不可操作（不显示操作按钮） 1：可操作（显示操作按钮
			if (
				row.groupState == 1 &&
				(!row.actDiscountId ||
					(row.actDiscountId && actDiscountId == row.actDiscountId))
			) {
				if (remove) {
					return '移除'
				} else {
