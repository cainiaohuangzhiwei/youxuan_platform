...
import { getWeixinUserListByCondition } from '@/api/coupon'
export default {
	components: {
		YxDialog,
		YxForm,
	...
	data() {
		return {
			...
			dialogAction: [
				{
					...
					type: 'primary',
					click: () => {
						this._getWeixinUserListByCondition(true)
					},
				},
			],
			tialogColumns: [
			...
			tialogPage: {
				...
				currentPage: 1,
				tableChange: () => {
					this._getWeixinUserListByCondition()
				},
			},
			tialogData: [],
		}
	...
	methods: {
		...
		saveBrand() {
			this.$emit('saveBrand', this.selectGroupList, 2)
			this.localVisible = false
		},
		findIndex(row) {
			// 判断是否已经添加用户
		...
		async _getWeixinUserListByCondition(flag) {
			// 用户列表
