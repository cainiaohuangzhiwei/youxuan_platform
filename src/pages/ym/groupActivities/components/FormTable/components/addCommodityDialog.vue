...
import { goodsPriceRange } from '@/api/groupbuying'
	...
	watch: {
		localVisible(val) {
			...
			if (val) {
				...
				// this.selectedBusiness.supplierAccountNumberId = this.dialogData.businessId
	...
	methods: {
		...
		async handleAdd() {
			...
			if (this.selectionData && this.selectionData.length) {
				...
				if (addId.length) {
					...
					} = await goodsPriceRange({
						goodsIdList: addId,
					})
					if (resultCode == 0) {
						//查询规格价格区间
						resultData.priceRangeList.forEach((item) => {
							this.selectGroupList.forEach((v) => {
								if (item.goodsId == v.goodsId) {
									v = Object.assign(v, item)
								}
							})
						})
						console.log('this.selectGroupList', this.selectGroupList)
					}
				}
			}
		},
		...
		async editUserList(row) {
			...
			if (ids.indexOf(row.goodsId) == -1) {
				...
				if (resultCode == 0) {
					row = Object.assign(row, resultData.priceRangeList[0])
					this.selectGroupList.push(row)
				}
			} else {
				this.selectGroupList.splice(ids.indexOf(row.goodsId), 1)
			}
			console.log('resultData', this.selectGroupList)
		},
		findUserParams(flag) {
