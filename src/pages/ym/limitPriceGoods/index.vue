	...
	<div class="goodsStockList">
		<basicContainer>
			...
			<Dialog
				...
				width="50%"
				:bottomBtn="viewDataBottonBtn"
				@before-close="viewDataLoding = false"
			>
				<div style="font-weight: bold; margin-bottom: 20px">
					下单抢购数量：{{ goodsTotalQuantity }}，有效订单数：{{
				...
				>
					...
					<template slot="orderSn" slot-scope="scope">
						<el-button @click="goToPage(scope.row)" type="text">{{
							scope.row.orderSn
						}}</el-button>
					</template>
				</yx-table>
...
export default {
	...
	data() {
		return {
			//查询列表权限
			secKillActivityGoodsQueryList: checkFunctionCode(
				'secKillActivityGoodsQueryList'
			),
			accSub,
			...
			formAction105: [
				...
				},
				{
					title: '导入添加活动商品',
					type: 'primary',
					auth: 'secKillActivityGoodsImportInfo',
					click: () => {
						this.$router.push({
							path: `/ym/limitPriceGoodsImport/${this.$route.params.secKillActivityId}/${this.$route.params.activityType}`,
						})
					},
					customRender: (action) => {
				...
				},
				{
					title: '导出商品信息',
					type: 'primary',
					auth: 'secKillActivityGoodsExport',
					click: () => {
				...
				},
				{
					title: '批量上架',
					type: 'primary',
					auth: 'secKillActivityGoodsBatchGoodsShelves',
					click: () => {
			...
			inventoryTable: [
				...
				},
				{
					...
					type: 'custom',
					dataIndex: 'killNum',
					hide: this.$route.params.activityType == 2,
				},
			],
			tableColumns106: [
				{
				...
				{
					type: 'string',
					dataIndex: 'saleKillNum',
					title: '已抢购数量',
					customRender: (row) => {
						return row.totalKillNum - row.killNum || '0'
					},
				},
				{
					type: 'string',
				...
				},
				{
					...
					actions: [
						...
						{
							title: '修改商品',
							auth: 'secKillActivityGoodsUpdateGoodsSpec',
							click: (row) => {
						...
						{
							title: '删除',
							auth: 'secKillActivityGoodsDeleteByIdInfo',
							click: (row) => {
	...
	methods: {
		...
		async quantityAudit() {
			...
			this.inventoryTableData.forEach((item, index) => {
				...
				// 如果数量未改变则killNum参数传null
				if (
					this.seckillQuantityList[index].goodsSpecId == item.goodsSpecId &&
					this.seckillQuantityList[index].killNum != item.killNum
				) {
					obj.killNum = item.killNum //可秒杀数量
				} else {
					obj.killNum = null //可秒杀数量
				}

				goodsSpecIdList.push(obj)
			})
			...
			try {
				this.inventoryTableData.map((item) => {
			...
			if (killNumVerify) {
				this.$message.warning({ message: '请输入正确的可抢购数量' })
				return
			}
			if (killPriceVerify) {
				this.$message.warning({ message: '请输入正确的活动价' })
				return
			}
		...
		getRecomposeGoods(row) {
			...
			this.inventoryTable[3].title =
				row.shelfState == 1 ? '可抢购数量' : '加可抢购数量（只增不减）'
		...
		_queryListGoods: async function (flag, e) {
			if (!this.secKillActivityGoodsQueryList) return
					...
					if (item.goodsSpecIdList && item.goodsSpecIdList.length) {
						item.goodsSpecIdList.forEach((v) => {
							const current = {
								...
								totalKillNum: v.totalKillNum, //总秒杀数量
								lowestPrice: v.lowestPrice, //销售价
								supplierName: item.supplierName, //商家名称
							}
			...
			this.$confirm('确定导出？', '提示', {
				...
				callback: async (action) => {
					if (action === 'confirm') {
						...
						downloadFile(data, '活动商品列表.xlsx')
					}
				},
			})
		},
		goToPage(row) {
			this.viewDataLoding = false
			this.$router.push({
