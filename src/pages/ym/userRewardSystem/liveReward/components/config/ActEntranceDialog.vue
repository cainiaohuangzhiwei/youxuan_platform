		...
		return {
			...
			formItems: [
				...
				},
				{
					type: 'uploadImage',
					title: '直播广场入口图',
					dataIndex: 'liveSquareEntranceImages',
					uploadAttributes: {
						limit: 1,
						imageName: 'imageUrl',
						httpRequest: (file) =>
							this._uploadImageHttpRequest(file, 'liveSquareEntranceImages'),
					},
				},
				{
					type: 'textarea',
	...
	methods: {
		...
		async open() {
			...
			if (resultCode == 0) {
				...
				if (
					resultData.liveSquareEntranceImages &&
					resultData.liveSquareEntranceImages.length > 0
				) {
					this.formData.liveSquareEntranceImages = resultData.liveSquareEntranceImages.map(
						(item) => {
							return {
								imageUrl: getBigImg(item),
								imageUrlTemp: item,
		...
		_uploadImageHttpRequest: async function (params, imageSource) {
			...
			if (
				...
				} else if (imageSource === 'liveSquareEntranceImages') {
					this.formData.liveSquareEntranceImages.push({
						imageUrl: getBigImg(res.data.resultData.imageList[0].imageUrl),
						imageUrlTemp: res.data.resultData.imageList[0].imageUrl,
					})
				}
		...
		async submitForm() {
			let params = {
				...
				liveSquareEntranceImages: this.formData.liveSquareEntranceImages.map(
					(item) => {
						return item.imageUrlTemp
					}
				),
