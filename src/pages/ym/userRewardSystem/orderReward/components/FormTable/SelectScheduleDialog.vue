<template>
	...
	>
		...
		<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
			...
			>
			<el-tab-pane :label="'已选档期' + selectGroupList.length" :name="2"
				><el-row>
...
import { getGoodsGroupListByConditionForCMS } from '@/api/userRewardSystem'
...
export default {
	...
	methods: {
			...
			if (this.selectionDataSd && this.selectionDataSd.length) {
				for (let i = 0; i < this.selectionDataSd.length; i++) {
					for (let j = 0; j < this.selectGroupList.length; j++) {
						if (
							this.selectionDataSd[i].goodsGroupId ===
		...
		async handleSelected() {
			...
			if (resultCode == 0) {
				this.selectedtialogData = resultData.goodsGroupList
				// this.selectedtialogPage.count = resultData.goodsGroupCount
				this.selectedcurrentTime = resultData.currentTime
			} else {
