 ...
 * @LastEditTime: 2021-11-18 15:40:35
...
request.interceptors.request.use(
	(config) => {
		if (config.url.indexOf('/yhtplus/webImport') == 0) {
			config.url = `https://${window.location.host}/weeget-infinite-backend-gateway/yhtplus-import${config.url}`
		}
		if (
			process.env.NODE_ENV == 'development' &&
			config.url.indexOf(
				`https://${window.location.host}/weeget-infinite-backend-gateway`
			) == 0
		) {
			config.url = config.url.replace(`https://${window.location.host}`, '')
		}
...
request.interceptors.response.use(
	(response) => {
		...
		if (response.data.resultCode != 0) {
			if (
				...
				if (response.data.resultMsg === '服务异常' && response.data.detailMsg) {
	...
	(error) => {
		...
		// 错误提示-接口返回的resultMsg
		if (Reflect.has(custom, 'showErrorMsg')) {
			Message.error(error.response.data.resultMsg || error.message)
		} else {
