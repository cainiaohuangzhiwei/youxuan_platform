define(['_config', 'beans', 'printHelper'], function (_config, beans, printHelper) {
  return {
    ...
    PrintHelper: {
      ...
      groupPurchaseOrderId: getQueryStringArgs().groupPurchaseOrderId,
      init: function (globalThis) {
        var _this = this

        globalThis.printHelper.callPrintConfig(function (printer) {
          _this.printConfiguration = printer = printer.purchasePrintConfiguration
      ...
      callTemplateOrderData: function (printConfiguration, fn) {
        ...
        switch (printConfiguration[templateTypeKey]) {
          case TEMPLATE_TYPE.BILL_80mm:// 80mm
            jsonFileName = 'printGroupOrderData-A4.json'
            break
          case TEMPLATE_TYPE.BILL_110mm:// 110mm
            jsonFileName = 'printGroupOrderData-A4.json'
            break
    ...
    PrintA4: {
      ...
      init: function (_globalThis, printConfiguration) {
        ...
        _this.addTemplate()
        _this.callgroupPurchaseOrders(globalThis)
      ...
      setTitleName: function () {
        ...
        if (!getQueryStringArgs().isPreview) {
          ...
          organizationName = getUserInfo().organizationName + '采购单'
          $('#page-title').html(organizationName)
          // });
        } else {
          organizationName = '广州微革' + '采购单'
          $('#page-title').html(organizationName)
        }
      },
      ...
      /* 获取订单详情 */
      callgroupPurchaseOrders: function (globalThis) {
        var _this = this
        var paramJson = {
          groupPurchaseOrderId: _this.groupPurchaseOrderId
        }
        $('#tips-cell').show(300)

        if (!getQueryStringArgs().isPreview) {
          sendPost(url.getGroupPurchaseOrderDetailByGroupPurchaseOrderId, {data: JSON.stringify(paramJson)}, function (resultJson) {
            setData(resultJson)
          })
        } else {
        ...
        function setData (resultJson) {
          var groupPurchaseOrder = resultJson.resultData.groupPurchaseOrder || {}

          fillDataToView($('body'), groupPurchaseOrder, function (key, value, itemView) {
          ...
          if (!isNull(groupPurchaseOrder.organizationAccountName)) {
            organizationAccount += groupPurchaseOrder.organizationAccountName
          }
          if (!isNull(groupPurchaseOrder.organizationSubbranchBank)) {
            organizationAccount += '-' + groupPurchaseOrder.organizationSubbranchBank
          }
          if (!isNull(groupPurchaseOrder.organizationAccountNo)) {
            organizationAccount += '-' + groupPurchaseOrder.organizationAccountNo
          }
          $('#organizationAccount').html(organizationAccount)

          // 设置收货信息
          // var consigneeInfo=$.groupPurchaseOrder.getConsigneeInfo(groupPurchaseOrder);
    ...
    Print110Or80mm: {
      ...
      initTable: function (resultJson) {
        ...
        if (_this.printConfiguration.sizeType == PRINT_SIZE_TYPE.NORMAL_110) {
          ...
          printOrderGoods = resultJson.resultData.gpoGoodsList
        }
    ...
    PrintGoods: {
      ...
      initTable: function (printGpoGoods) {
        ...
        $.each(printGpoGoods, function (i, itemJson) {
          ...
          var gpoGoodsList = itemJson.gpoGoodsList || 0
          if (gpoGoodsList.length > 0) {
            var specItem = gpoGoodsList[0]
