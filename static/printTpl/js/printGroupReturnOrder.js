define(['_config', 'beans', 'printHelper'], function (_config, beans, printHelper) {
  return {
    ...
    PrintHelper: {
      ...
      groupPurchaseReturnOrderId: getQueryStringArgs().groupPurchaseReturnOrderId,
      init: function (globalThis) {
        var _this = this

        globalThis.printHelper.callPrintConfig(function (printer) {
          _this.printConfiguration = printer = printer.purchaseReturnPrintConfiguration
      ...
      callTemplateOrderData: function (printConfiguration, fn) {
        ...
        switch (printConfiguration[templateTypeKey]) {
          case TEMPLATE_TYPE.BILL_80mm: // 80mm
            jsonFileName = 'printGroupReturnOrderData-A4.json'
            break
          case TEMPLATE_TYPE.BILL_110mm: // 110mm
            jsonFileName = 'printGroupReturnOrderData-A4.json'
            break
    ...
    PrintA4: {
      ...
      setTitleName: function () {
        ...
        if (!getQueryStringArgs().isPreview) {
          ...
          organizationName = getUserInfo().organizationName + '采购退货单'
          $('#page-title').html(organizationName)
          //                  });
        } else {
          organizationName = '广州微革' + '采购退货单'
          $('#page-title').html(organizationName)
        }
      },
      ...
      initTable: function (printGpoGoods, newSpecTwoList) {
        var _this = this
        var dip = js_getDPI()[0]
        ...
        $.each(printGpoGoods, function (i, itemJson) {
          ...
          if (itemJson.minPriceOne == itemJson.maxPriceOne) {
            $lastChild.find('.priceOne').html(itemJson.minPriceOne)
          } else {
            $lastChild.find('.priceOne').html(itemJson.minPriceOne + '-' + itemJson.maxPriceOne)
          }
      ...
      callOrderDetails: function (globalThis) {
        var _this = this
        var paramJson = {
          groupPurchaseReturnOrderId: _this.groupPurchaseReturnOrderId
        }
        $('#tips-cell').show(300)

        if (!getQueryStringArgs().isPreview) {
          sendPost(url.getGroupPurchaseReturnOrderDetailByReturnOrderId, {
        ...
        function setData (resultJson) {
          var groupPurchaseReturnOrder = resultJson.resultData.groupPurchaseReturnOrder || {}
          if (groupPurchaseReturnOrder.isPriceChanged == 1) {
            $('#isPriceChanged').hide()
          }
          fillDataToView($('body'), groupPurchaseReturnOrder, function (key, value, itemView) {
            ...
            if (key == 'moLingAmount') {
              if (isNull(value) || value == 0) {
                $('*[data-fill="moLingAmount"]').parents('span:first').hide()
                return ''
              }
            }
            if (key == 'remark') {
          ...
          // 设置组织logo
          try {
            $('#company-logo').attr('src', getCompanyInfo().image.imageUrl)
          } catch (e) {}
          var organizationAccount = ''
          if (!isNull(groupPurchaseReturnOrder.organizationAccountName)) {
            organizationAccount += groupPurchaseReturnOrder.organizationAccountName
          }
          if (!isNull(groupPurchaseReturnOrder.organizationSubbranchBank)) {
            organizationAccount += '-' + groupPurchaseReturnOrder.organizationSubbranchBank
          }
          if (!isNull(groupPurchaseReturnOrder.organizationAccountNo)) {
            organizationAccount += '-' + groupPurchaseReturnOrder.organizationAccountNo
          }
          $('#organizationAccount').html(organizationAccount)
          // 设置收货信息
          //                  var consigneeInfo=$.groupPurchaseReturnOrder.getConsigneeInfo(groupPurchaseReturnOrder);
          ...
          sendPost(url.getGoodsConfiguration, {}, function (configResultJson) {
            ...
            _this.initTable(resultJson.resultData.printGpoGoods, newSpecTwoList)
    ...
    Print110Or80mm: {
      ...
      initTable: function (resultJson) {
        ...
        if (_this.printConfiguration.sizeType == PRINT_SIZE_TYPE.NORMAL_110) {
          ...
          $('#table-dashed-line').css({
            'border-bottom': 'none'
          })
          printOrderGoods = resultJson.resultData.printGpoGoods
          ...
          printOrderGoods = resultJson.resultData.gproGoodsList
        }
        ...
        $.each(printOrderGoods, function (i, itemJson) {
          ...
          if (_this.printConfiguration.sizeType == PRINT_SIZE_TYPE.NORMAL_110) {
            ...
            $lastChild.find('.goodsNumber').html(goodsString)
            $lastChild.find('.totalAmount').html(itemJson.totalAmount.toFixed(2))
        ...
        $('span[data-fill="totalCount"]').html(totalQunatity)
      },
        ...
        function setData (resultJson) {
          ...
          fillDataToView($('body'), groupPurchaseReturnOrder, function (key, value, itemView) {
            if (key.indexOf('Time') != -1) {
              ...
              value = value || 0
              return new Date(value).Format('yyyy-MM-dd hh')
            }
            if (key == 'otherAmountRemark') {
              if (isNull(value)) {
                return '其他费用'
    ...
    PrintGoods: {
      ...
      initTable: function (printGpoGoods) {
        var _this = this
        var dip = js_getDPI()[0]
        // 毫米转px
        function mm2Px (inPx) {
        ...
        $.each(printGpoGoods, function (i, itemJson) {
          ...
          var gproGoodsList = itemJson.gproGoodsList || 0
          if (gproGoodsList.length > 0) {
            var specItem = gproGoodsList[0]
            var rowTotalAmount = NumberTool.floatMul(specItem.price, specItem.quantity)
            ...
            $lastChild.find('.totalAmount').html(rowTotalAmount.toFixed(2))
            totalAmount += rowTotalAmount
            totalQunatity += specItem.quantity
          }
        ...
        if (_this.hasGoodsImage) {
          $('#table-container').find('.table-box:last').append($('#t-goodsimg-table-footer-item').html())
        } else {
          $('#table-container').find('.table-box:last').append($('#t-goods-table-footer-item').html())
        }
        $('.goods-table-total-amount').html(totalAmount.toFixed(2))
      ...
      callOrderDetails: function (globalThis) {
        ...
        function setData (resultJson) {
          ...
          fillDataToView($('body'), groupPurchaseReturnOrder, function (key, value, itemView) {
            ...
            if (key == 'otherAmountRemark') {
              if (isNull(value)) {
                $('*[data-fill="otherAmountRemark"]').parents('span:first').hide()
                return ''
              } else {
                return '（' + value + '）'
              }
