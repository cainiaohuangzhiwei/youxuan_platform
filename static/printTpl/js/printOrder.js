define(['_config', 'beans', 'printHelper'], function (_config, beans, printHelper) {
  return {
    ...
    PrintHelper: {
      ...
      // 是否显示上次欠款
      isShowLastUnpaid: function () {
        var _this = this
        if (!isNull(_this.printConfiguration)) {
          return _this.printConfiguration.printContainLastTimeUnpaidAmount == 1
        }
        return false
      },
      // 是否显示收款方式
      isShowPaymentType: function () {
        var _this = this
        if (!isNull(_this.printConfiguration)) {
          return _this.printConfiguration.printContainPaymentType == 1
        }
        return false
      },
      // 获得订单模板数据
      callTemplateOrderData: function (printConfiguration, fn) {
        ...
        switch (printConfiguration[templateTypeKey]) {
          case TEMPLATE_TYPE.BILL_80mm: // 80mm
            jsonFileName = 'printOrderData-80mm.json'
            break
          case TEMPLATE_TYPE.BILL_110mm: // 110mm
            jsonFileName = 'printOrderData-80mm.json'
            break
          case TEMPLATE_TYPE.DOUBLE_SKU: // 双规格
            jsonFileName = 'printOrderData-A4.json'
            break
          case TEMPLATE_TYPE.SINGLE_SKU_IMG: // 单规格有图
            jsonFileName = 'printOrderData-A4.json'
            break
    ...
    PrintA4: {
      ...
      init: function (_globalThis, printConfiguration) {
        ...
        // 是否显示上次欠款
        if (globalThis.PrintHelper.isShowLastUnpaid()) {
          $('#lastUnpaidBox').show()
        } else {
          $('#lastUnpaidBox').hide()
        }
        // 是否显示收款方式
        if (globalThis.PrintHelper.isShowPaymentType()) {
          $('#paymentTypeBox').show()
        } else {
          $('#paymentTypeBox').hide()
        }
      ...
      callOrderDetails: function (globalThis) {
        ...
        if (!getQueryStringArgs().isPreview) {
          sendPost(url.getOrderDetailByOrderIdPlus, {
        ...
        function setData (resultJson) {
          ...
          // 设置上次收款方式
          var orderPayment = resultJson.resultData.orderPayment
          if (isNull(orderPayment)) {
            $('#paymentType').html('-')
          } else {
            $('#paymentType').html($.orderPayment.getPayTypeByCode(orderPayment.paymentType))
          }
          ...
          sendPost(url.getGoodsConfiguration, {}, function (configResultJson) {
            ...
            _this.initTable(resultJson.resultData.printOrderGoods, newSpecTwoList)
    ...
    Print110Or80mm: {
      ...
      initTable: function (resultJson) {
        ...
        if (_this.printConfiguration.sizeType == PRINT_SIZE_TYPE.NORMAL_110) {
          ...
          printOrderGoods = resultJson.resultData.printOrderGoods
          ...
          printOrderGoods = resultJson.resultData.orderGoodsList
        }
    ...
    PrintGoods: {
      ...
      initTable: function (printOrderGoods) {
        ...
        $.each(printOrderGoods, function (i, itemJson) {
          ...
          var orderGoodsSpecList = itemJson.orderGoodsSpecList || 0
