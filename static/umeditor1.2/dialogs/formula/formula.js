	...
	UM.registerWidget('formula', {
		...
		initContent: function (_editor, $widget) {
			...
			$.each(formula, function (k, v) {
				...
				$.each(v, function (i, f) {
					contentHtml.push(
						'<li class="edui-formula-latex-item" data-latex="' +
							f +
				...
				xMax = 0
				$tabContent.append(
					'<div class="edui-tab-pane"><ul>' + contentHtml.join('') + '</ul>'
				)
				headHtml.push(
					'<li class="edui-tab-item"><a href="javascript:void(0);" class="edui-tab-text">' +
						lang['lang_tab_' + k] +
						'</a></li>'
				)
			})
			...
			$root
				...
				.append('<div class="edui-formula-clearboth"></div>')
		...
		switchTab: function (index) {
			var me = this,
				...
				index = $.isNumeric(index)
					? index
					: $.inArray(index, $root.find('.edui-tab-nav .edui-tab-item'))

			$root
				.find('.edui-tab-nav .edui-tab-item')
				.removeClass('edui-active')
				.eq(index)
				.addClass('edui-active')
			$root
				.find('.edui-tab-content .edui-tab-pane')
				.removeClass('edui-active')
				.eq(index)
				.addClass('edui-active')
