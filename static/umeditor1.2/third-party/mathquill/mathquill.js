...
;(function () {
	...
	var Fragment = P(function (_) {
		_.init = function (leftEnd, rightEnd) {
			...
			pray(
				'leftEnd and rightEnd have the same parent',
  ...
  */
	...
	if (transformPropName) {
		...
		scale = function (jQ, x, y) {
			...
			if (!jQ.hasClass('matrixed-container')) {
				jQ.addClass('matrixed-container').wrapInner(
	...
	LatexCmds.underline = bind(
		...
		'\\underline',
		'span',
		'class="non-leaf underline"'
	)
	LatexCmds.overline = LatexCmds.bar = bind(
		Style,
		'\\overline',
		'span',
		'class="non-leaf overline"'
	)
	...
	var SupSub = P(MathCommand, function (_, _super) {
		...
		_.respace = function () {
			if (
				this[L].ctrlSeq === '\\int ' ||
				(this[L] instanceof SupSub &&
					this[L].ctrlSeq != this.ctrlSeq &&
					this[L][L] &&
			...
			if (this.respaced) {
				...
				this.jQ.css({
					left:
						(this.limit && this.ctrlSeq === '_' ? -0.25 : 0) -
						leftWidth / fontSize +
						'em',
	...
	LatexCmds.superscript = LatexCmds.supscript = LatexCmds['^'] = bind(
		SupSub,
		'^',
		'sup',
		'**'
		...
		_.createLeftOf = function (cursor) {
			if (!this.replacedFragment) {
				...
				if (leftward !== cursor[L]) {
					this.replaces(
						MathFragment(leftward[R] || cursor.parent.ends[L], cursor[L])
					)
					cursor[L] = leftward
				}
			}
	...
	LatexCmds.langle = LatexCmds.lang = bind(
		Bracket,
		'&lang;',
		'&rang;',
		'\\langle ',
	...
	var CloseBracket = P(Bracket, function (_, _super) {
		_.createLeftOf = function (cursor) {
			...
			if (
				...
				cursor.parent.parent.end === this.end &&
	...
	LatexCmds.rangle = LatexCmds.rang = bind(
		CloseBracket,
		'&lang;',
		'&rang;',
		'\\langle ',
	...
	LatexCmds.rbrack = LatexCmds.rbracket = CharCmds[']'] = bind(
		CloseParen,
		'[',
		']'
	)

	var Pipes = (LatexCmds.lpipe = LatexCmds.rpipe = CharCmds['|'] = P(
		Paren,
		function (_, _super) {
			_.init = function () {
				_super.init.call(this, '|', '|')
	...
	var TextBlock = (CharCmds.$ = LatexCmds.text = LatexCmds.textnormal = LatexCmds.textrm = LatexCmds.textup = LatexCmds.textmd = P(
		MathCommand,
		function (_, _super) {
			_.ctrlSeq = '\\text'
	...
	LatexCmds.textsc = makeTextBlock(
		'\\textsc',
		'span',
		'style="font-variant:small-caps" class="text"'
	)
	LatexCmds.uppercase = makeTextBlock(
		'\\uppercase',
		'span',
		'style="text-transform:uppercase" class="text"'
	)
	LatexCmds.lowercase = makeTextBlock(
		'\\lowercase',
		'span',
		'style="text-transform:lowercase" class="text"'
	)
	...
	var LatexCommandInput = (CharCmds['\\'] = P(MathCommand, function (
		...
		_.htmlTemplate =
			'<span class="latex-command-input non-leaf">\\<span>&0</span></span>'
		_.textTemplate = ['\\']
		_.createBlocks = function () {
	...
	LatexCmds.f = bind(
		Symbol,
		'f',
		'<var class="florin">&fnof;</var><span style="display:inline-block;width:0">&nbsp;</span>'
	)
	...
	var NonSymbolaSymbol = P(Symbol, function (_, _super) {
		_.init = function (ch, html) {
			_super.init.call(
				this,
				ch,
				'<span class="nonSymbola">' + (html || ch) + '</span>'
			)
		}
	})
	...
	var BinaryOperator = P(Symbol, function (_, _super) {
		_.init = function (ctrlSeq, html, text) {
			_super.init.call(
				this,
				ctrlSeq,
				'<span class="binary-operator">' + html + '</span>',
				text
			)
		}
	})
	...
	LatexCmds.notin = LatexCmds.sim = LatexCmds.cong = LatexCmds.equiv = LatexCmds.oplus = LatexCmds.otimes = P(
		BinaryOperator,
		function (_, _super) {
			_.init = function (latex) {
				_super.init.call(this, '\\' + latex + ' ', '&' + latex + ';')
...
*/
	...
	LatexCmds.closecurlybrace = bind(
		VanillaSymbol,
		'\\closecurlybrace ',
		'&#125;'
	)
	...
	LatexCmds.inf = LatexCmds.infin = LatexCmds.infty = LatexCmds.infinity = bind(
		VanillaSymbol,
		'\\infty ',
		'&infin;'
	)
	...
	;(function () {
		...
		for (var i in trig) {
			LatexCmds[trig[i]] = LatexCmds[trig[i] + 'h'] = LatexCmds[
				'a' + trig[i]
			] = LatexCmds['arc' + trig[i]] = LatexCmds[
				'a' + trig[i] + 'h'
			] = LatexCmds['arc' + trig[i] + 'h'] = NonItalicizedFunction
		}
	})()
...
JS environment could actually contain many instances. */
	...
	var Cursor = P(Point, function (_) {
				...
				do {
					...
					if (prop) {
						...
						if (prop === false || prop instanceof MathBlock) {
							self.upDownCache[ancestorBlock.id] = Point(
								self.parent,
								self[L],
								self[R]
							)
		...
		_.selectFrom = function (anticursor) {
			...
			loopThroughAncestors: while (true) {
				...
				for (
					var otherI = anticursor;
					otherI !== otherA.parent.parent;
					otherI = otherI.parent.parent
				)
			...
			this.hide().selection = Selection(
				left[L][R] || left.parent.ends[L],
				right[R][L] || right.parent.ends[R]
			)
	...
	jQuery(function () {
		jQuery('.mathquill-editable:not(.mathquill-rendered-math)').mathquill(
			'editable'
		)
		jQuery('.mathquill-textbox:not(.mathquill-rendered-math)').mathquill(
			'textbox'
		)
